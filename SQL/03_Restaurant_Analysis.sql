-- Zomato Restaurant Analytics — restaurant analysis
-- Fresher-level SQL analysis of city, pricing, ratings and digital adoption.

USE zomato_analytics;

-- 1. Restaurant count and average metrics by city
SELECT
    city,
...
-- 4. Online ordering and table reservation by city
SELECT
    city,
    COUNT(*) AS restaurant_count,
    ...
    ROUND(100 * AVG(table_reservation), 2) AS reservation_pct
FROM zomato_restaurants_clean
GROUP BY city
HAVING COUNT(*) >= 100
ORDER BY restaurant_count DESC;
...
-- Note: This dataset does not contain order-level revenue or customer data.
-- Cost-for-two is a pricing field and is not revenue.
